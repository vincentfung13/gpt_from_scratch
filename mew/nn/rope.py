...
class RotaryPositionalEmbedding(nn.Module):
    def __init__(self, theta: float, d_head: int, max_seq_len: int, device=None):
        ...
        # freqs -> (max_seq_len, d_head_half)
        freqs = einsum(
            inds, self.inverse_freq, "seq_len, d_head_half -> seq_len d_head_half"
        )
    ...
    def _update_cache(self, max_seq_len: int):
        ...
        freqs = einsum(
            ...
            "seq_len, d_head_half -> seq_len d_head_half",
        )
        self.register_buffer("sin", freqs.sin())
        self.register_buffer("cos", freqs.cos())
    ...
    def forward(self, x: torch.Tensor, token_positions: torch.Tensor) -> torch.Tensor:
        ...
        x_rearr = rearrange(
            x,
            "... seq_len (d_head_half d2) -> ... seq_len d_head_half d2",
            d_head_half=self.d_head // 2,
            d2=2,
        )

        # Extract the two components of each pair
        x0 = x_rearr[..., 0]  # (..., seq_len, d_head_half)
        x1 = x_rearr[..., 1]  # (..., seq_len, d_head_half)
        ...
        return rearrange(
            [x0_new, x1_new],
            "d2 ... seq_len d_head_half -> ... seq_len (d_head_half d2)",
        )
